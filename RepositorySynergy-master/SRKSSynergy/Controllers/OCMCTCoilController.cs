...
{
    public class OCMCTCoilController : Controller
    {
        ...
        [HttpPost]
        public ActionResult Create(OCMCTCoil motor)
        {
            ...
            db.OCMCTCoil.Add(motor);
            db.SaveChanges();

            return RedirectToAction("Index", "OCMCTCoil", null);
        }

        public ActionResult Index()
        {
            var list = db.OCMCTCoil.Where(m => m.IsDeleted == 0).ToList();
            return View(list);
        }
        ...
        {
            OCMCTCoil motor = db.OCMCTCoil.Find(id);
            if (motor == null)
            {
                return HttpNotFound();
            }
        ...
        public ActionResult Edit(OCMCTCoil motor)
        {
            var cpid = Convert.ToInt32(Session["logincpid"]);
        ...
        {
            OCMCTCoil ts = db.OCMCTCoil.Find(id);
            if (ts == null)
            {
                return HttpNotFound();
            }
        ...
        [HttpPost]
        public ActionResult Delete(OCMCTCoil am1, int id)
        {
            if (Request.Form["Yes"] != null)
            {
                OCMCTCoil am = db.OCMCTCoil.Find(id);
                am.IsDeleted = 1;
