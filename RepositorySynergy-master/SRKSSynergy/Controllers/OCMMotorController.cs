...
{
    public class OCMMotorController : Controller
    {
        ...
        [HttpPost]
        public ActionResult Create(OCMMotor motor)
        {
            ...
            db.OCMMotor.Add(motor);
            db.SaveChanges();

            return RedirectToAction("Index","OCMMotor",null);
        }

        public ActionResult Index()
        {
            var list = db.OCMMotor.Where(m => m.IsDeleted == 0).ToList();
            return View(list);
        }
        ...
        {
            OCMMotor motor = db.OCMMotor.Find(id);
            if (motor == null)
            {
                return HttpNotFound();
            }
        ...
        public ActionResult Edit(OCMMotor motor)
        {
            var cpid = Convert.ToInt32(Session["logincpid"]);
        ...
        {
            OCMMotor ts = db.OCMMotor.Find(id);
            if (ts == null)
            {
                return HttpNotFound();
            }
        ...
        [HttpPost]
        public ActionResult Delete(OCMMotor am1, int id)
        {
            if (Request.Form["Yes"] != null)
            {
                OCMMotor am = db.OCMMotor.Find(id);
                am.IsDeleted = 1;
